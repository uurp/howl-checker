...
Logger.log(f"Howl Checker started at {time()}")
...
{g}  --------------------------------------------------{x}
      ...
      {r}@{x}  Howl — Discord Username Checker (Tokenless)     
      {r}@{x}  GitHub: https://github.com/uurp                    
      {r}@{x}  Version: {VERSION}                                 
      {r}@{x}  Platform: Cross-compatible (Win/Linux)             
...
def TITLE_SPINNER():
    ...
    TITLE = ["Howl", "Avaible : {WORKS}", "Taken : {TAKEN}", "Requests : {REQUESTS}", "RPS : {RPS}", "Elapsed : {ELAPSED}s"]
    while True:
        for i in TITLE:
            edited = i          
            for _ in range(50):
...
def WEBHOOK_PROCESSOR():
    ...
    while True:
        ...
        if len(names_diff) > 1 and last_send_time-time() < 5: #if we get hits frequently and user is not scaning for super rare names we switch to batch mode
            ...
            json = {"content": "\n".join(payload), 'username': 'howl', 'avatar_url': 'https://t4.ftcdn.net/jpg/11/34/20/75/360_F_1134207559_iHGnaWs7MUEw10gZGz81V2krG0TTPMuR.jpg'}
            ...
            json = {"content": message.replace("<name>", name).replace("<time>", str(hittime)).replace("<elapsed>", str(round(current_time - start_time))).replace("<RPS>", str(RPS)), 'username': 'Howl', 'avatar_url': 'https://upload.wikimedia.org/wikipedia/commons/thumb/9/95/Burning_Yellow_Sunset.jpg/1280px-Burning_Yellow_Sunset.jpg'}
            x = CLOUDCHECKER.session.post(
                url=webhook,
                json=json
            )
